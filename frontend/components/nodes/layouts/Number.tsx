...
const WorkflowNumber: React.FC<WorkflowNumberProps> = React.memo(
  ({ number, setNumber, label, type = "int" }) => {
    ...
    const [inputValue, setInputValue] = useState<string>(number.toString());
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (type === "float" && e.key === "." && !e.currentTarget.value.includes(".")) return;
    ...
    return (
      <div
        className={cn(
          "bg-node-input rounded-[4px] py-[4px] px-[8px] gap-[5px] flex text-node-text hover:text-node-text-hover text-xs focus-within:bg-node-input-focus w-full"
        )}
      >

...
export default function AddMenu() {
  ...
  const handleMouseDown = (e: MouseEvent) => {
    if (e.button === 2) {
      ...
      if (!canvasRef.current) return; // TODO EDITOR et pas workflow
      const editorBound = canvasRef.current.getBoundingClientRect();

      setPosition({
        x: e.clientX - editorBound.x,
        y: e.clientY - editorBound.y,
      });
    } else {
  ...
  useEffect(() => {
    canvasRef.current = document.getElementById("canvas") as HTMLDivElement;

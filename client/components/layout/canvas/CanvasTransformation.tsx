...
function CanvasTransformation({
  ...
  const [pivotPoint, setPivotPoint] = React.useState<{
    x: number;
    y: number;
  } | null>(null);
  const [defaultPositionState, setDefaultPositionState] = React.useState<{
    x: number;
    y: number;
  } | null>(null);
  ...
  useLayoutEffect(() => {
    triggerRef.current = draggableRef.current?.querySelector(
      trigger
    ) as HTMLDivElement;
  ...
  useEffect(() => {
    ...
    const handleWheelNative = (e: WheelEvent) => {
      if (!e.ctrlKey) return; // ignore si Ctrl non pressé
      e.preventDefault(); // bloque zoom natif

      const editor = document.getElementById("editor") as HTMLDivElement;
      if (!editor || !defaultPositionState || isDragging) return;
      ...
      setPosition((prev) => ({
        x: prev.x - offsetX * (newScale - scale),
        y: prev.y - offsetY * (newScale - scale),
      }));
  ...
  }, [scale, defaultPositionState, isDragging]);
    ...
    (e: MouseEvent) => {
      ...
      setPosition((prevPosition) => {
        if (pivotPoint && isDragging) {
          const x =
            prevPosition.x +
            (e.clientX - pivotPoint!.x - draggable.getBoundingClientRect().x);
          const y =
            prevPosition.y +
            (e.clientY - pivotPoint!.y - draggable.getBoundingClientRect().y);
          return { x, y };
        } else {
          return prevPosition;
        }

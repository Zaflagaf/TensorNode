...
# Suppose que X_train et y_train sont des numpy arrays ou tensors
train_dataset = tf.data.Dataset.from_tensor_slices((X_train, y_train))
train_dataset = train_dataset.batch(batch_size)  # <-- ici tu précises le batch size

# 2. Boucle d'entraînement manuelle
for epoch in range(epochs):
    print(f"Epoch {epoch+1}")

    for step, (x_batch, y_batch) in enumerate(X_train):
        with tf.GradientTape() as tape:
            """ ici précision de la manière d'entrainement """
